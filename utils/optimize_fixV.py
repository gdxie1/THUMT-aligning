...
def create_train_op(loss, optimizer, global_step, params):
    with tf.name_scope("create_train_op"):
        var_list = []
        # here we only change the context varibale_scope
        # var_list.extend(tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
        #                                  "transformer/decoder"))
        var_list.extend(tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
                                          "transformer/aligning"))
        # var_list.extend(tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
        #                                   "transformer/src_embedding"))
        # # two specific variable
        # var_list.extend(tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
        #                                   "transformer/softmax_embedding"))
        # var_list.extend(tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
        #                                   "transformer/target_embedding"))

        grads_and_vars = optimizer.compute_gradients(
            loss, var_list, colocate_gradients_with_ops=True)
