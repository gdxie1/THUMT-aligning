...
import transformer_aligning

import utils.hooks as hooks
import utils.optimize_fixV as optimize
...
def restore_trained_variables(checkpoint):
    #  load all the variables of standard transformer model
    if not checkpoint:
        return tf.no_op("restore_trained_op")
    ...
    for (name, shape) in var_list:
        ...
        name = name.split(":")[0]
        #transformer/bias transformer/source_embedding 在新的模型下，挪到了transformer/src_embedding scope 下面
        if name == "transformer/bias":
            name = "transformer/src_embedding/bias"
        elif name == "transformer/source_embedding":
            name = "transformer/src_embedding/source_embedding"
        elif name == "transformer/softmax":
            name = "transformer/softmax_embedding"
        values[name] = tensor
...
def main(args):
    ...
    model_cls = transformer_aligning.Transformer
    ...
    with tf.Graph().as_default():
        ...
        def restore_fn(step_context):
            step_context.session.run(restore_trained_op)
            step_context.session.run(restore_ctx_op)
            print("run restore_func")
        ...
        def step_fn(step_context):
            ...
            #                           "transformer/decoder/layer_0/self_attention/layer_norm/scale")
