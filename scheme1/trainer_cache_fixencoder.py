...
def restore_encoder_variables(thumt_checkpoint):
    if not thumt_checkpoint:
        return tf.no_op("restore_encoder_op")

    # Load checkpoints
    tf.logging.info("Loading encoder form %s" % thumt_checkpoint)
    var_list = tf.train.list_variables(thumt_checkpoint)
    reader = tf.train.load_checkpoint(thumt_checkpoint)
    values = {}
    ...
    encoder_var_list = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
                                         "transformer/encoder")
    ...
    for var in encoder_var_list:
        name = var.name.split(":")[0]

        if name in values:
            tf.logging.info("Restore vars of encoder %s" % var.name)
            ops.append(tf.assign(var, values[name]))

    return tf.group(*ops, name="restore_encoder_op")
...
def main(args):
    ...
    with tf.Graph().as_default():
        ...
        restore_trained_encoder_op = restore_encoder_variables(args.thumt_checkpoint)
        # Validation
